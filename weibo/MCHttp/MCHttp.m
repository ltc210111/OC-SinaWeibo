...
+ (void)postRequestURLStr:(NSString *)urlStr
    ...
    [[self alloc] requestWithUrl:urlStr parameters:parameters requsetType:RequestTypePost isCache:isCache imageKey:nil withData:nil loadProgress:^(float progress)  {
...
+ (void)getRequestURLStr:(NSString *)urlStr
                    ...
                    success:(SuccessBlock)success
                    failure:(ErrorBlock)failure{
    [[self alloc] requestWithUrl:urlStr parameters:parameters requsetType:RequestTypeGet isCache:isCache imageKey:nil withData:nil loadProgress:^(float progress)  {
...
+ (void)getRequestCacheURLStr:(NSString *)urlStr
                         ...
                         success:(SuccessBlock)success
                         failure:(ErrorBlock)failure{
    [[self alloc] requestWithUrl:urlStr parameters:parameters requsetType:RequestTypeGet isCache:YES imageKey:nil withData:nil  loadProgress:^(float progress)  {
...
- (void)requestWithUrl:(NSString *)url
    ...
    if (requestType == RequestTypeGet) {
        [HYBNetworking getWithUrl:url params:parameters progress:^(int64_t bytesRead, int64_t totalBytesRead) {
            if (loadProgress) {
                float myProgress = (float)bytesRead / (float)totalBytesRead;
                loadProgress(myProgress);
            }
        } success:^(id response) {
            [self dealwithResponseObject:response cacheUrl:cacheUrl cacheData:cacheData isCache:isCache success:^(NSDictionary *responseObject,NSString * msg) {
                if(success){
                   success(responseObject,msg);
                }
            } failure:^(NSString *errorInfo) {
    ...
    if (requestType == RequestTypePost) {
        [HYBNetworking postWithUrl:url params:parameters progress:^(int64_t bytesRead, int64_t totalBytesRead) {
            if (loadProgress) {
                float myProgress = (float)bytesRead / (float)totalBytesRead;
                loadProgress(myProgress);
            }
    ...
    if (requestType == RequestTypeUpLoad) {
        ...
        [HYBNetworking uploadFileWithUrl:url uploadingFile:attach progress:^(int64_t bytesWritten, int64_t totalBytesWritten) {
            float myProgress = (float)bytesWritten / (float)totalBytesWritten;
            loadProgress(myProgress);
        } success:^(id response) {
            [self dealwithResponseObject:response cacheUrl:cacheUrl cacheData:cacheData isCache:NO success:^(NSDictionary *responseObject,NSString * msg) {
                if(success){
                    success(responseObject,msg);
                }
            } failure:^(NSString *errorInfo) {
...
+ (void)initialize{
    NSString * bundleName =[[NSBundle mainBundle] infoDictionary][(NSString *)kCFBundleIdentifierKey];
